---
title: "Data Analysis_HB_17052016"
...
load("Data_17052016.RData") # without those who already knew Julia Verlinden
```
...
ggplot(df, aes(treatment, Donation)) +
  ...
  #stat_summary(fun.y = median, colour="red", geom="point", shape=18, size=3) +
...
```{r, echo=FALSE}
wilcox.test(df$Donation ~ df$RecvsDefD, alternative = "less")
```

*Based on the one-sided Wilcoxon test we do not reject the null that Donations in default treatments are equal to donations in recommendation treatments.*
...
```{r, echo=FALSE}
wilcox.test(df$Donation ~ df$RecNapvsRecNos, exact = FALSE, alternative = "greater")
```
...
```{r, echo=FALSE}
wilcox.test(df$Donation ~ df$DefNapvsDefNos, exact = FALSE, alternative = "greater")
```
...
wilcox.test(df$Donation[df$treatment != "Control"] ~ df$ReactD[df$treatment != "Control"], alternative = "greater")
```

*Based on the (one-sided) Wilcoxon test we can not reject the null hypothesis of a location shift of the donation amount equal to zero of those with an above vs. below/equal median reactance score, among those that encountered either type of intervention (aggregated over/ irrespective of sourcetype).*
...
```{r, echo=FALSE} 
wilcox.test(df$Donation[df$ReactD == "Above Med React"] ~ df$RecvsDefD[df$ReactD == "Above Med React"], alternative = "greater")
lm <- (lm(Donation ~ Sourcetype + RecvsDefD * ReactD, df))
coeftest(lm, vcov = vcovHC(lm, "HC1"))
lm <- (lm(Donation ~ Sourcetype * RecvsDefD * ReactD, df))
coeftest(lm, vcov = vcovHC(lm, "HC1"))

ggplot(df, aes(x = ReactD, y = Donation)) +
  facet_wrap(Sourcetype~RecvsDef)+
  geom_boxplot() +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=2) +
  theme(axis.text.x = element_text(angle = 45, vjust=.5, size=10))

ggplot(data = dfsub, aes(x = Sourcetype, y = Donation, color = ReactD)) +
...
*The OLS regression with robust standard errors suggests that there is no interaction effect between having above median reactance score and the default on donation amount. When including the tripple interaction with Sourcetype and Intervention type some parameters become significant, but this could be due to the same problem of not many observations per category as above. Judged by the latter figure, there might be an interaction for recommendation instrument when switching from political to partisan source information with reactance above/ or below median. For recommendations, switching from political to partisan source information increases average/median donation amounts for those with below or equal to median reactance, and decreases average/median donation amounts for those with above median reactance. This and other interaction effects are appearent in the triple interaction robust OLS regression above.*
...
```{r}
wilcox.test(df$Donation ~ df$RecNapvsRecKno, exact = FALSE, alternative = "less")
```

*Based on the Wilcoxon test we cannot confidently reject the null hypothesis of zero donation location shift between recommendation groups with name and picture information vs. recommendation groups with knowledge source information in favor of the alternative hypothesis that the former are less than the latter.* 

### For Defaults: Donations in Knowledge treatments > Donations in Name and Picture treatments
```{r}
wilcox.test(df$Donation ~ df$DefNapvsDefKno, exact = FALSE, alternative = "less")
```

*Based on the Wilcoxon test we cannot confidently reject the null hypothesis of zero donation location shift between default groups with name and picture information vs. default groups with knowledge source information in favor of the alternative hypothesis that the former are less than the latter.* 
...
```{r}
wilcox.test(df$Donation ~ df$RecNapvsRecPol, exact = FALSE, alternative = "greater")
```

*Based on the one sided Wilcoxon test we cannot reject the null hypothesis that Donations in recommendation treatments informing about the political mandate of the source are equal to donations in recommendations treatments providing the name and picture of the source, in favor of the alternative hypothesis that donations in recommendation + political mandate groups are less than those in the recommendation + name and picture groups.*

### For Defaults: Donations in Political treatments < Donations in Name and Picture treatments
```{r}
wilcox.test(df$Donation ~ df$DefNapvsDefPol, exact = FALSE, alternative = "greater")
```

*Based on the one sided Wilcoxon test cannot reject the null that donations in default treatments informing about the political mandate of the source are equal to donations in default treatments providing the name and picture of the source, in favor of the alternative hypothesis that donations in default + political mandate groups are less than those in the default + name and picture groups.*
...
```{r}
wilcox.test(df$Donation ~ df$RecPolvsDefPol, exact = FALSE, alternative = "greater")
```

*Based on the one sided Wilcoxon test we cannot reject the null that donations in default treatments informing about the political characteristics of the source are equal to donations in recommendation treatments informing about the political characteristics of the source, in favor of the alternative hypothesis that donations are higher in the political recommendation groups than in the political default groups.*
...
ggplot(data=df, aes(x = trustPolM, y = Donation, color = RecPolvsDefPol)) +
  facet_wrap(~RecPolvsDefPol) +
  geom_boxplot() + 
  geom_jitter() +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  theme(axis.text.x = element_text(angle = 45, vjust=.5, size=10)) +
  ggtitle(expression(atop("Relationship between trust in politics above M dummy and Donation", atop(italic("resp. for RecPol and DefPol treatment groups"),""))))

wilcox.test(df$Donation[df$treatment == "RecPol"] ~ df$trustPolM[df$treatment == "RecPol"], exact = FALSE, alternative = "less")
wilcox.test(df$Donation[df$treatment == "DefPol"] ~ df$trustPolM[df$treatment == "DefPol"], exact = FALSE, alternative = "less")

lm <- (lm(Donation ~ RecPolvsDefPol * trustPolM, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```

*The one sided Wilcoxon tests suggest that there is no effect of having trust above median neither for those in the RecPol, nor for those in the DefPol treatment groups. I think this does not exclude a possible interaction effect per se, but the OLS regression suggests that, among political sources, there is no significant interaction effect of above median trust in politics and receiving a recommendation vs. default by a political actor on donation amount.*
...
ggplot(data=df, aes(x = conformityM, y = Donation, color = RecPolvsDefPol)) +
  facet_wrap(~RecPolvsDefPol) +
  geom_boxplot() + 
  geom_jitter() +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  theme(axis.text.x = element_text(angle = 45, vjust=.5, size=10)) +
  ggtitle(expression(atop("Relationship between conformity above M dummy and Donation", atop(italic("resp. for RecPol and DefPol treatment groups"),""))))

wilcox.test(df$Donation[df$treatment == "RecPol"] ~ df$conformityM[df$treatment == "RecPol"], exact = FALSE, alternative = "less")
wilcox.test(df$Donation[df$treatment == "DefPol"] ~ df$conformityM[df$treatment == "DefPol"], exact = FALSE, alternative = "less")

lm <- (lm(Donation ~ RecPolvsDefPol * conformityM, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```

*The one sided Wilcoxon tests suggest that there is an effect of valuing conformity above median for those in the RecPol, confidently rejecting the null hypothesis of no true locations hift in favor of the alternative hypothesis that donations for those in the RecPol treatment are lower then valuing conformity less than the median. The test is not significant for those in the DefPol treatment groups. The OLS regression with robust standard errors suggests that, among subjects encountering political sources, there is a significant interaction effect of above median conformity valuation and seeing a default. The main effects are not significant. Based on the sign of the estimate, having above conformity valuation decreases the (positive?) effect of having a default by a political source instead of a recommendation by a political source.*
...
ggplot(data=df, aes(x = green, y = Donation, color = RecParvsDefPar)) +
  ...
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  theme(axis.text.x = element_text(angle = 45, vjust=.5, size=10)) +
  ggtitle(expression(atop("Relationship between green party affiliation and Donation", atop(italic("resp. for RecPar and DefPar treatment groups"),""))))

wilcox.test(df$Donation[df$treatment == "RecPar"] ~ df$green[df$treatment == "RecPar"], exact = FALSE, alternative = "less")
wilcox.test(df$Donation[df$treatment == "DefPar"] ~ df$green[df$treatment == "DefPar"], exact = FALSE, alternative = "less")

lm <- (lm(Donation ~ RecParvsDefPar * green, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))

```

*The one sided Wilcoxon tests suggest that there is an effect of having green party preferences for those in the DefPol treatment group, confidently rejecting the null hypothesis of no true location shift in favor of the alternative hypothesis that donations for those in the DefPol treatment are lower when being not green. The test is not significant for those in the RecPol treatment groups. The OLS regression with robust standard errors suggests that there are neither main effects nor an interaction effect.*
...
*Based on the two-sided Wilcoxon test we can not confidently reject the null hypothesis of equal median locations of donation amounts between those who believe we cooperated with Julia Verlinden and those who do not believe us.*
...
wilcox.test(df$Donation ~ df$NonPolvsPol, alternative = "less")
...
lm <- (lm(Donation ~ RecvsDefD * Sourcetype * gender, dfsub))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```
...
* *According to the robust logistic regression incorporating a triple interaction term of Intervention type, sourcetype and gender, being female increases the effect of a partisan source on average donations.*
...
ggplot(data = dfsub, aes(x = Sourcetype, y = Donation)) +
...
* *There appears to be a minor effect of providing source information for women, when the intervention type is a recommendation. There, providing information about the political/partisan characteristics of the source increases median and mean donations.*
...
* *Providing information about the political/partisan characteristics of the source for defaults (not for recommendations) appearantly increases the likelihood to donate for men. However, this does not appear to increase average donations for this group (previous boxplot).*
* *Providing information about the polical/partisan characteristics of the source appearantly neither increases the probability to donate for recommendations nor for defaults, for women. However, providing women with political/partisan source information for recommendations increases average donations.*

```{r, echo = FALSE}
kruskal.test(dfsub$Donation[dfsub$gender == "Weiblich" & dfsub$Intervention == "Recommendation"] ~ dfsub$Sourcetype[dfsub$gender == "Weiblich" & dfsub$Intervention == "Recommendation"])
```

*Based on the Kruskal Wallis test, average Donations of women seeing a recommendation differ significantly for the different sourcetypes.*
...
*Neither for women seeing a default, nor for men seeing either intervention type, is there an effect of sourcetype on average donations.*

```{r, echo = FALSE}
kruskal.test(dfsub$Donation[dfsub$gender == "Weiblich" & dfsub$Intervention == "Recommendation"] ~ dfsub$NonPolvsPolA[dfsub$gender == "Weiblich" & dfsub$Intervention == "Recommendation"]) 

lm <- (lm(Donation ~ gender*RecvsDefD*NonPolvsPol, dfsub))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```

* *Based on the Kruskal Wallis test, there is a significant difference of donations between women seeing a recommendation by non-political/partisan vs. policial/partisan sources. This is a more aggregated effect than the effect before. Before, we saw that this effect is significant primarily because of the donations in the recommendation by partisan source groups (which has, by the way, 17 observations, i.e. there are 17 womeon that see a recommendation by a partisan source).*

## Gender and Reactance interaction per treatment
```{r, echo=FALSE}
ggplot(data = dfsub, aes(x = Reactance, y = Donation, color = gender)) +
  facet_grid(~gender) +
  geom_jitter() +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=2) +
  ylab("Average donation amount and mean donations") +
  xlab("Reactance score")
...
*The Kruskal Wallis test is not significant. This means that there are also no significant differences in Donations for those that believed we cooperated with Julia Verlinden.*
...
```{r, echo=FALSE}
ggplot(data = df, aes(x = Sourcetype, y = Donation)) +
...
```{r, echo=FALSE}
ggplot(data = df, aes(x = SourcetypeD, y = Donation)) +
...
```{r, echo=FALSE}
wilcox.test(df$Donation ~ df$warmGlowD)
...
lm <- (lm(Donation ~ RecvsDefD * Sourcetype + warmGlowD, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
#lm <- (lm(Donation ~ RecvsDefD * Sourcetype * warmGlowD, df))
#coeftest(lm, vcov = vcovHC(lm,type="HC1"))
...
* *Based on the Wilcoxon test, average donations differ between groups that feel or do not feel warm glow when contributing to climate change.*
...
* *Based on the robust OLS regression, warm Glow is highly positively significant as a predictor for average Donations, when controlling for treatments and interactions, which themselves are all insignificant (Which they had been before anyway). When including warm Glow as a part of a tripple interaction term, every predictor becomes insignificant (not reported here).*
...
wilcox.test(df$Donation ~ df$retireEffD) 
```

*Based on the Wilcoxon test we can confidently reject the null hypothesis of equal median donations (Assuming similar distributions) of those that judge carbon license destruction as an efficient measure to fight climate change vs. does who do not.*
...
ggplot(data = df, aes(Sourcetype, Donation)) +
  facet_grid(retireEffD~Intervention) +
  geom_boxplot() +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  theme(axis.text.x = element_text(angle = 90, vjust=.8, size=10))
lm <- (lm(Donation ~ RecvsDef * Sourcetype * retireEffD, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```
...
wilcox.test(df$Donation ~ df$moralD)
```
...
```{r, echo=FALSE}
lm <- (lm(Donation ~ treatment + moralD, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
lm <- (lm(Donation ~ RecvsDefD * Sourcetype * moralD, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```

* *When controlling for treatments, moral duty is a significant positive predictor for the Donation amount.*
...
```{r, echo = FALSE}
ggplot(data = df, aes(x = income, y = Donation)) +
  geom_jitter()
lm <- (lm(Donation ~ treatment+income, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
glm <- (glm(Donated ~ treatment+income, df, family = "binomial"))
coeftest(glm, vcov = vcovHC(glm,type="HC1"))
```

*Reported income is neither a significant predictor for the average donation amount, nor for the probability to donate anything.*
...
ggplot(df, aes(KnovsPol, Donation)) +
  facet_grid(~RecvsDefD) +
  geom_boxplot()
```

*There is no interaction effect of Knowledge vs political source information and intervention type on the probability to donate anything. There is also no interaction on the average donation amount (regression not reported here).*
...
#ggplot(data = df, aes(x = Sourcetype, y = Donation, color = EmotionRespM)) +
#  facet_grid(EmotionRespM~RecvsDef) +
#  geom_boxplot() +
#  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=2) +
#  theme(axis.text.x = element_text(angle = 45, vjust=.5, size=10))
#lm <- (lm(Donation ~ RecvsDef * Sourcetype * EmotionRespM, df))
#coeftest(lm, vcov = vcovHC(lm,type="HC1"))
#lm <- (lm(Donation ~ RecvsDef * Sourcetype * ReactComplM, df))
#coeftest(lm, vcov = vcovHC(lm,type="HC1"))
#lm <- (lm(Donation ~ RecvsDef * Sourcetype * ResistInflM, df))
#coeftest(lm, vcov = vcovHC(lm,type="HC1"))
#lm <- (lm(Donation ~ RecvsDef * Sourcetype * ReactAdvM, df))
#coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```
...
* *Kruskal Wallis Test suggests that experiencing a demand effect has an impact on Donation amount.*
...
b <- boxcox(lm(Donation ~ moralD + retireEffD + NosvsSomeD * RecvsDefD * Reactance + gender + age + income + belief, dfsub)) #because of the 0s, we have to do the transformation with Donation+1
lamda <- b$x
lik <- b$y
bc <- cbind(lamda, lik)
...
df$logD <- log(df$Donation+100)
log(2)
```

# Separate into two regressions (because bimodal): Logit then OLS (without zeros)
```{r, echo = FALSE}
ggplot(data = df, aes(x = Donationno0)) +
  geom_histogram() +
  stat_bin(binwidth = 1) +
  xlab("Donation amount for subjects that donated a positive amount")
```

*The distribution of average donations excluding those that gave nothing is also not perfectly normally distributed (because of the small peack at 1 Euro), but more so than the original variable. It is left skewed.*

```{r, echo = FALSE}
ggplot(data = df, aes(x = Donationno0)) +
  geom_density()
## MIND HERE THAT THIS WOULD ALSO HAVE TO CHANGE WHEN I DECIDE TO LOOK AT THE DISTANCE TO 5 INSTEAD OF THE DONATION AMOUNT
...
#lm <- lm(Donationno0 ~ ..., dfsub)
```

# All tests and descriptive statistics with Distance to 5 instead of Donation variable
