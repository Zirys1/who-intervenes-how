---
title: "Data Analysis_HB_20052016 - DISTcen"
...
load("Data_02062016.RData") # without those who already knew Julia Verlinden
```
...
1. Distcen, which is the amount the subject donated in order to retire emission rights
...
## Variable: Distcen to retire carbon licenses

> Distcen is coded as the Distance of the Donation to re recommended/defaulted value (Donation of 5 would be Distance of -5), whereas all Distances > 0 are coded as 0 Distances.

### Aggregated descriptive statistics
```{r, echo=FALSE}
summary(df$Distcen)
sd(df$Distcen)
ggplot(data = df, aes(x = Distcen)) +
...
### Distcenribution of aggregated Distcens
```{r, echo=FALSE, message=FALSE} 
ggplot(df, aes(x = Distcen)) +
geom_histogram() + 
stat_bin(binwidth = .1) +
  ggtitle("Frequency of Distcen values") +
  xlab("Distcen amount") +
  ylab("Frequency")
```

### Distcenribution of aggregated Distcens with normal curve

```{r, echo = FALSE}
x <- df$Distcen
h <- hist(x, breaks=10, col = "red", xlab = "Distcen amount")
...
### Kernel density plot of aggregated Distcens
...
### Distcenribution of Distcens by treatment
...
ggplot(data=df, aes(x=Distcen, color=treatment)) +
  facet_wrap(~treatment) +
  stat_bin(binwidth = .1) +
  geom_text(data=df.n, aes(x=5, y = 10, label=V1), parse=TRUE) +
  ggtitle("Frequency of Distcen values by treatment") +
  xlab("Distcen amount") +
  ylab("Frequency") 
```

### Distcens by treatment (Boxplot)
*Red diamonds in boxplots represent the respective means*

```{r, echo=FALSE}
# generate dataframe with Distcen average for each treatment
ggplot(df, aes(treatment, Distcen)) +
  geom_boxplot() +
  geom_jitter(shape=1) +
...
*There do not appear to be big differences in the median Distcens between the different treatments. Differences in mean Distcens, which also d not appear to be pretty big, seem to be largely driven by outliers.*

```{r}
kruskal.test(df$Distcen ~ df$treatment)
```

*The Kruskal Wallis test does not reject the null hypothesis of no differences in Distcens between treatments.*

### Distcens by aggregated treatment (Boxplot), i.e. Def vs. Rec vs. Control

```{r, echo=FALSE}
ggplot(df, aes(as.factor(RecvsDef), Distcen)) +
  geom_boxplot() +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
...
*Aggregated over all different types of sources, there appears to be no difference between Distcens, when the intervention is a recommendation vs. when the intervention is a default. Furthermore, both seem not to be significantly different from the control group, especially when considering that the higher means in the recommendation and default groups are likely due to outliers.*

```{r, echo = FALSE}
kruskal.test(df$Distcen, df$RecvsDef)
```

*The Kruskal Wallis test does not reject the null hypothesis of no differences in Distcens between recommendation, default, and control groups, irrespective of the source of intervention.*

### Distcens by Reactance score 
...
Shows a point plot (not jittered) with Distcen amount and the respective Reactance score of each participant. Includes a linear regression line, including the 95% confidence region, of the Reactance score as a predictor for the Distcen amount.

```{r, echo=FALSE}
ggplot(data=df, aes(x = Reactance, y = Distcen)) +
  geom_point() +
  geom_smooth(method="lm")
```

*Irrespective of treatments (i.e. assuming that reactance scores are randomly Distcenributed over different treatments (no correlation)) there is no (linear) relationship between Distcen amount and reactance score.*

#### Distcens by Reactance score per treatment
Shows a point plot (not jittered) with Distcen amount and the respective Reactance score of each participant, for each treatment. Includes a linear regression line, including the 95% confidence region, of the Reactance score as a predictor for the Distcen amount, for each treatment.

```{r, echo=FALSE} 
ggplot(data=df, aes(x = Reactance, y = Distcen, color = treatment)) +
  facet_wrap(~treatment) +
  geom_point() +
  geom_text(data=df.n, aes(x=5, y=10, label=V1), parse=TRUE)+
  geom_smooth(method="lm")
...
*The Kruskal Wallis test does not reject the null hypothesis that there is no difference between the reactance scores in the different treatment groups. The plots show that there does not seem to be a consistent relationship between Distcen amount and reactance score over the different treatments. Judging by the graphical representation of the 95% confidence interval of the regression line, reactance score (treated as a metric) seems to be a bad predictor for Distcen amount, even when controlling for treatments.*
*This becomes apparent in the following regression (With heteroskedasticity robust standard errors).*

```{r, echo=FALSE}
lm <- (lm(Distcen ~ treatment + Reactance, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```



### Distcens by EAI score
...
Shows a point plot (not jittered) with Distcen amount and the respective EAI score of each participant. Includes a linear regression line, including the 95% confidence region, of the EAI score as a predictor for the Distcen amount.

```{r, echo=FALSE}
ggplot(data=df, aes(x = EAI, y = Distcen)) +
  geom_point() +
  geom_smooth(method="lm")
```

*Irrespective of treatments (i.e. assuming that EAI scores are randomly Distcenributed over different treatments (no correlation)) there appears to be a slightly positive relationship between EAI and Distcen amount. Mind, however, that, since EAI was surveyed after the experiment, it is not undoubtedly sure that there is a causal relationship. We would need to assume that Distcens do not influence responses to the EAI. In any case, EAI score can be regarded as a predictor of Distcen amount (irrespective of treatments), that appears to be slightly positive.*

#### Distcens by EAI score per treatment
Shows a point plot (not jittered) with Distcen amount and the respective EAI score of each participant, for each treatment. Includes a linear regression line, including the 95% confidence region, of the EAI score as a predictor for the Distcen amount, for each treatment.

```{r, echo=FALSE} 
ggplot(data=df, aes(x = EAI, y = Distcen, color = treatment)) +
  facet_wrap(~treatment) +
  geom_point() +
  geom_text(data=df.n, aes(x=5, y=10, label=V1), parse=TRUE) +
  geom_smooth(method="lm")
...
*The Kruskal Wallis test does not reject the null hypothesis that there is no difference between the EAI scores in the different treatment groups. The plots show that there seems to be a consistent relationship between Distcen amount and EAI score over the different treatments. Judging by the graphical representation of the 95% confidence interval of the regression line, EAI score (treated as a metric) seems to be a positive, but not necessarily significant predictor for Distcen amount, even when controlling for treatments.*
*This becomes apparent in the following regression (robust SE), where, when controlling for treatments, EAI is a significantly positive predictor for Distcen amount.*

```{r, echo=FALSE}
lm <- (lm(Distcen ~ treatment + EAI, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```

## Variable: Distcen dummy (1 if donated, 0 otherwise)
### Aggregated descriptive statistics
```{r, echo=FALSE}
summary(df$Donated)
...
*Visually, there appears to be no relationship of reactance score and Distcen broken down by treatment on the decision to donate.*
...
*Visually, there appears to be some positive relationship of EAI score and Distcen broken down by treatment on the decision to donate. However, it is hard to verify visually.*

## Variable: Beliefs about other participants Distcens
### Aggregated descriptive statistics
```{r, echo=FALSE}
summary(df$belief)
sd(df$belief)
...
### Distcenribution of aggregated beliefs about Distcens
```{r, echo=FALSE, message=FALSE} 
ggplot(df, aes(x = belief)) +
geom_histogram() + 
stat_bin(binwidth = .5) +
  ggtitle("Frequency of beliefs about others' Distcens") +
  xlab("Belief") +
  ylab("Frequency")
...
### Distcenribution of beliefs by treatment
```{r, echo=FALSE, message=FALSE}
ggplot(data=df, aes(x=belief, color=treatment)) +
  facet_wrap(~treatment) +
  stat_bin(binwidth = .1) +
  geom_text(data=df.n, aes(x=4, y = 10, label=V1), parse=TRUE) +
  ggtitle("Frequency of beliefs about others Distcens by treatment") +
  xlab("Belief") +
  ylab("Frequency")
```
...
*There do not appear to be differences in the median beliefs about other participants Distcens between the different treatments. The Kruskal Wallis test supports this view and does not reject the null hypothesis of equal median beliefs between the different treatments.*
...
*H0: Average Distcens = 0*

*HA: Average Distcens >< 0*
```{r, echo=FALSE}
wilcox.test(df$Distcen, mu = 0)
```

*Based on the Wilcoxon test we reject the null that Distcens are equal to 0. Note however that the test almost rejects any value. This is possibly because of the metric nature of the Distcen variable.*
...
### Aggregated Distcens in recommendation treatments < Distcens in control group
```{r, echo=FALSE}
kruskal.test(df$Distcen, df$RecvsDef)
```

*Based on the Kruskal Wallis test we do not reject the null hypothesis that Distcens do not significantly differ between groups that get a recommendation, a default, or are in the control group.*

### Aggregated Distcens in default treatments < Distcens in recommendation treatments
```{r, echo=FALSE}
wilcox.test(df$Distcen ~ df$RecvsDefD, alternative = "greater")
```

*Based on the one-sided Wilcoxon test we do not reject the null that Distcens in default treatments are equal to Distcens in recommendation treatments in favour of the alternative hypothesis that the Distcenance to 5 is higher in the recommendation treatments than in the default treatments.*
...
### For Recommendations: Distcens in Name and Picture treatments < Distcens in No-Source treatments
```{r, echo=FALSE}
wilcox.test(df$Distcen ~ df$RecNapvsRecNos, exact = FALSE, alternative = "less")
```

*Based on the Wilcoxon test we do not reject the null hypothesis that Distcens in recommendation treatments informing about the name and picture of the source are equal to Distcens in recommendation treatments providing no information about the source of the recommendation.*

### For Defaults: Distcens in Name and Picture treatments < Distcens in No-Source treatments
```{r, echo=FALSE}
wilcox.test(df$Distcen ~ df$DefNapvsDefNos, exact = FALSE, alternative = "less")
```

*Based on the Wilcoxon test we do not reject the null hypothesis that Distcens in default treatments informing about the name and picture of the source are equal to Distcens in default treatments providing no information about the source of the default, since p > .05.*
...
Distcen <- lm(Distcen ~ RecvsDefD * Sourcetype * Reactance, df)
Distcen <- coeftest(Distcen, vcov = vcovHC(Distcen, "HC1"))
...
ggplot(data=df, aes(x = Reactance, y = Distcen)) +
  ...
  geom_point() +
  geom_smooth(method="lm") +
  ggtitle(expression(atop("Relation between Distcen and Reactance score", atop(italic("resp. for Rec and Def treatment groups"),""))))

wilcox.test(df$Distcen[df$treatment != "Control"] ~ df$ReactD[df$treatment != "Control"], alternative = "greater")
```

*Based on the (one-sided) Wilcoxon test we can not reject the null hypothesis of a location shift of the Distcen amount equal to zero of those with an above vs. below/equal median reactance score, among those that encountered either type of intervention (aggregated over/ irrespective of sourcetype).*
...
**This is an interaction of Reactance covariate with the interventiontype-factor on the Distcenance to the recommended/defaulted value.**

```{r, echo=FALSE} 
wilcox.test(df$Distcen[df$ReactD == "Above Med React"] ~ df$RecvsDefD[df$ReactD == "Above Med React"], alternative = "less")
lm <- (lm(Distcen ~ Sourcetype + RecvsDefD * ReactD, df))
coeftest(lm, vcov = vcovHC(lm, "HC1"))
lm <- (lm(Distcen ~ Sourcetype * RecvsDefD * ReactD, df))
coeftest(lm, vcov = vcovHC(lm, "HC1"))

ggplot(df, aes(x = ReactD, y = Distcen)) +
...
ggplot(data = df, aes(x = Sourcetype, y = Distcen)) +
...
*The OLS regression with robust standard errors suggests that there is no interaction effect between having above median reactance score and the default on Distcen amount. When including the tripple interaction with Sourcetype and Intervention type some parameters become significant, but this could be due to the same problem of not many observations per category as above. Judged by the latter figure, there might be an interaction for recommendation instrument when switching from political to partisan source information with reactance above/ or below median. For recommendations, switching from political to partisan source information increases average/median Distcen amounts for those with below or equal to median reactance, and decreases average/median Distcen amounts for those with above median reactance. This and other interaction effects are appearent in the triple interaction robust OLS regression above.*
...
### For Recommendations: Distcens in Knowledge treatments > Distcens in Name and Picture treatments
```{r}
wilcox.test(df$Distcen ~ df$RecNapvsRecKno, exact = FALSE, alternative = "greater")
```

*Based on the Wilcoxon test we cannot confidently reject the null hypothesis of zero Distcen location shift between recommendation groups with name and picture information vs. recommendation groups with knowledge source information in favor of the alternative hypothesis that the former are greater than the latter.* 

### For Defaults: Distcens in Knowledge treatments > Distcens in Name and Picture treatments
```{r}
wilcox.test(df$Distcen ~ df$DefNapvsDefKno, exact = FALSE, alternative = "greater")
```

*Based on the Wilcoxon test we cannot confidently reject the null hypothesis of zero Distcen location shift between default groups with name and picture information vs. default groups with knowledge source information in favor of the alternative hypothesis that the former are greater than the latter.* 
...
### For Recommendations: Distcens in Political treatments < Distcens in Name and Picture treatments
```{r}
wilcox.test(df$Distcen ~ df$RecNapvsRecPol, exact = FALSE, alternative = "less")
```

*Based on the one sided Wilcoxon test we cannot reject the null hypothesis that Distcens in recommendation treatments informing about the political mandate of the source are equal to Distcens in recommendations treatments providing the name and picture of the source, in favor of the alternative hypothesis that Distcens in recommendation + political mandate groups are greater than those in the recommendation + name and picture groups.*

**Two sided test has a p-value = 0.3506**

### For Defaults: Distcens in Political treatments < Distcens in Name and Picture treatments
```{r}
wilcox.test(df$Distcen ~ df$DefNapvsDefPol, exact = FALSE, alternative = "less")
```

*Based on the one sided Wilcoxon test cannot reject the null that Distcens in default treatments informing about the political mandate of the source are equal to Distcens in default treatments providing the name and picture of the source, in favor of the alternative hypothesis that Distcens in default + political mandate groups are greater than those in the default + name and picture groups.*
...
### Distcens in default treatments informing about the political characteristics of the source > Distcens in recommendation treatments informing about the political characteristics of the source
```{r}
wilcox.test(df$Distcen ~ df$RecPolvsDefPol, exact = FALSE, alternative = "less")
```

*Based on the one sided Wilcoxon test we cannot reject the null that Distcens in default treatments informing about the political characteristics of the source are equal to Distcens in recommendation treatments informing about the political characteristics of the source, in favor of the alternative hypothesis that Distcens are lower in the political recommendation groups than in the political default groups.*

*Test the hypothesis when interpreting partisan also as political:*
```{r, echo=FALSE}
wilcox.test(df$Distcen[df$Sourcetype == "Political" | df$Sourcetype == "Partisan"] ~ df$RecvsDefD[df$Sourcetype == "Political" | df$Sourcetype == "Partisan"], alternative = "less")
```
...
**This is an interaction effect of interventiontype-factor with trust in politics covariate on the Distcenance to the recommended/defaulted value.**
...
```{r, echo = FALSE}
ggplot(data=df, aes(x = trustPolD, y = Distcen)) +
  facet_wrap(~RecPolvsDefPol) +
  geom_boxplot() +
  geom_jitter(shape=1) +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  ggtitle(expression(atop("Relationship between trust in politics dummy and Distcen", atop(italic("resp. for RecPol and DefPol treatment groups"),""))))
```
...
```{r, echo=FALSE}
ggplot(data=df, aes(x = trustPolM, y = Distcen)) +
  facet_grid(SourcetypeD~RecvsDefD) +
  geom_boxplot() + 
  geom_jitter(shape=1) +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  theme(axis.text.x = element_text(angle = 45, vjust=.5, size=2)) +
  ggtitle(expression(atop("Relationship between trust in politics above M dummy and Distcen", atop(italic("for factor interaction"),""))))
...
ggplot(data=df, aes(x = trustPolM, y = Distcen)) +
  ...
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  theme(axis.text.x = element_text(angle = 45, vjust=.5, size=10)) +
  ggtitle(expression(atop("Relationship between trust in politics above M dummy and Distcen", atop(italic("resp. for RecPol and DefPol treatment groups"),""))))

wilcox.test(df$Distcen[df$treatment == "RecPol"] ~ df$trustPolM[df$treatment == "RecPol"], exact = FALSE, alternative = "two.sided")
wilcox.test(df$Distcen[df$treatment == "DefPol"] ~ df$trustPolM[df$treatment == "DefPol"], exact = FALSE, alternative = "two.sided")

lm <- (lm(Distcen ~ Sourcetype * RecvsDefD * trustPolM, df))
coeftest(lm, vcov = vcovHC(lm, "HC1"))
```

*The one sided Wilcoxon tests suggest that there is no effect of having trust above median neither for those in the RecPol, nor for those in the DefPol treatment groups. I think this does not exclude a possible interaction effect per se, but the OLS regression suggests that, among political sources, there is no significant interaction effect of above median trust in politics and receiving a recommendation vs. default by a political actor on Distcen amount.*
...
ggplot(data=df, aes(x = trustPolM, y = Distcen)) +
  ...
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  theme(axis.text.x = element_text(angle = 45, vjust=.5, size=10)) +
  ggtitle(expression(atop("Relationship between trust in politics above M dummy and Distcen", atop(italic("resp. for RecPP and DefPP treatment groups"),""))))

wilcox.test(df$Distcen[df$treatment == "RecPol" | df$treatment == "RecPar"] ~ df$trustPolM[df$treatment == "RecPol" | df$treatment == "RecPar"], alternative = "two.sided")
wilcox.test(df$Distcen[df$treatment == "DefPol" | df$treatment == "DefPar"] ~ df$trustPolM[df$treatment == "DefPol" | df$treatment == "DefPar"], exact = FALSE, alternative = "two.sided")
describeBy(df$Distcen[df$treatment == "DefPol" | df$treatment == "DefPar"], df$trustPolM[df$treatment == "DefPol" | df$treatment == "DefPar"])

lm <- lm(Distcen ~ RecvsDefD * SourcetypeD * trustPolM, df) # dunno if this is right
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```

*The second Wilcoxon Test suggests that , among subjects seeing either of the two political sourcetypes, the Distcenance to 5 is significantly different between those that have bewlo or equal to median trust in politics and those that have above median trust in politics (p < .1). Judged by the boxplots, having above median trust in politics rather tha below or equal to median, decreases the Distcenance to the suggested value of five when the intervention type is a default.*
...
ggplot(data=df, aes(x = conformityD, y = Distcen)) +
  facet_wrap(~RecPolvsDefPol) +
  geom_boxplot() +
  geom_jitter(shape=1) +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  ggtitle(expression(atop("Relationship between conformity dummy and Distcen", atop(italic("resp. for RecPol and DefPol treatment groups"),""))))

wilcox.test(df$Distcen ~ df$conformityD)
```
...
ggplot(data=df, aes(x = conformityM, y = Distcen)) +
  facet_grid(RecvsDefD~SourcetypeD) +
  geom_boxplot() +
  geom_jitter(shape=1) +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  theme(axis.text.x = element_text(angle = 45, vjust=.5, size=2))+
  ggtitle(expression(atop("Relationship between conformity dummy and Distcen", atop(italic("resp. for factor combinations"),""))))
...
ggplot(data=df, aes(x = conformityM, y = Distcen)) +
  ...
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  theme(axis.text.x = element_text(angle = 45, vjust=.5, size=10)) +
  ggtitle(expression(atop("Relationship between conformity above M dummy and Distcen", atop(italic("resp. for RecPol and DefPol treatment groups"),""))))

wilcox.test(df$Distcen ~ df$conformityM)

lm <- lm(Distcen ~ RecvsDefD * Sourcetype * conformityM, df)
coeftest(lm, vcov = vcovHC(lm,type="HC1" ))
```
...
```{r, echo = FALSE}
ggplot(data=df, aes(x = green, y = Distcen)) +
  facet_wrap(~RecParvsDefPar) +
  geom_boxplot() + 
  geom_jitter(shape=1) +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  theme(axis.text.x = element_text(angle = 45, vjust=.5, size=10)) +
  ggtitle(expression(atop("Relationship between green party affiliation and Distcen", atop(italic("resp. for RecPar and DefPar treatment groups"),""))))

wilcox.test(df$Distcen[df$treatment == "RecPar"] ~ df$green[df$treatment == "RecPar"], exact = FALSE, alternative = "greater")
wilcox.test(df$Distcen[df$treatment == "DefPar"] ~ df$green[df$treatment == "DefPar"], exact = FALSE, alternative = "greater")

lm <- (lm(Distcen ~ RecvsDefD * Sourcetype * green, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))

```

*The one sided Wilcoxon tests suggest that there is an effect of having green party preferences for those in the DefPar treatment group, confidently rejecting the null hypothesis of no true location shift in favor of the alternative hypothesis that Distcens for those in the DefPar treatment are lower when being not green. The test is not significant for those in the RecPar treatment groups.*
...
### Variable: Distcen amount
```{r, echo=FALSE}
describeBy(df$Distcen, df$believe2)
wilcox.test(df$Distcen ~ df$believe2)
```

*Based on the two-sided Wilcoxon test we can not confidently reject the null hypothesis of equal median locations of Distcen amounts between those who believe we cooperated with Julia Verlinden and those who do not believe us.*
...
```{r, echo=FALSE, message=FALSE}
ggplot(data = df, aes(x = Intervention, y = Distcen, colour = Intervention)) +
  facet_wrap(~Sourcetype) +
  geom_boxplot() +
  xlab("Intervention type") +
  ylab("Average Distcen") +
  theme(axis.text.x = element_text(angle = 45, vjust=.5, size=10)) +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3)
...
ggplot(data = df, aes(x = Sourcetype, y = Distcen, colour = Sourcetype)) +
...
ggplot(data = df, aes(x = NonPolvsPol, y = Distcen, colour = NonPolvsPol)) +
  facet_wrap(~Intervention) +
  geom_boxplot() +
  ylab("Average Distcen") +
  xlab("Source type") +
...
describeBy(df$Distcen, df$NonPolvsPol)
wilcox.test(df$Distcen ~ df$NonPolvsPol, alternative = "less")

lm <- (lm(Distcen ~ NonPolvsPol*Intervention, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```
...
ggplot(data = df, aes(x = NonPolvsPolA, y = Distcen, color = Intervention)) +
  facet_grid(~Intervention) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, vjust=.8, size=10)) +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3)

kruskal.test(df$Distcen[df$Intervention == "Recommendation"] ~ df$NonPolvsPolA[df$Intervention == "Recommendation"])
kruskal.test(df$Distcen[df$Intervention == "Default"] ~ df$NonPolvsPolA[df$Intervention == "Default"])
```
...
## Average Distcen gender differences (by treatment)
```{r, echo=FALSE}
ggplot(dfsub, aes(x = gender, y = Distcen)) +
  geom_boxplot() +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3)

wilcox.test(dfsub$Distcen ~ dfsub$gender)

df.nu <- aggregate(dfsub$ID, list(dfsub$gender, dfsub$treatment), length)
colnames(df.nu) <- c("gender", "treatment", "x")
ggplot(data = dfsub, aes(x = gender, y = Distcen, color = gender )) +
  facet_wrap(~treatment) +
  geom_boxplot() +
...
kruskal.test(df$Distcen[df$gender == "Weiblich"] ~ df$treatment[df$gender == "Weiblich"])
kruskal.test(df$Distcen[df$gender == "Männlich"] ~ df$treatment[df$gender == "Männlich"])

lm <- (lm(Distcen ~ RecvsDefD * Sourcetype * gender, dfsub))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```
...
* *According to the robust logistic regression incorporating a triple interaction term of Intervention type, sourcetype and gender, being female increases the effect of a partisan source on average Distcens.*
...
```{r, echo = FALSE}
ggplot(data = dfsub, aes(x = gender, y = Distcen)) +
  facet_grid(~Intervention) +
  geom_boxplot() +
  xlab("Gender") +
  ylab("Distcen") +
...
ggplot(data = dfsub, aes(x = Sourcetype, y = Distcen)) +
...
* *There appears to be a minor effect of providing source information for women, when the intervention type is a recommendation. There, providing information about the political/partisan characteristics of the source increases median and mean Distcens.*
...
* *Providing information about the political/partisan characteristics of the source for defaults (not for recommendations) appearantly increases the likelihood to donate for men. However, this does not appear to increase average Distcens for this group (previous boxplot).*
* *Providing information about the polical/partisan characteristics of the source appearantly neither increases the probability to donate for recommendations nor for defaults, for women. However, providing women with political/partisan source information for recommendations increases average Distcens.*

```{r, echo = FALSE}
kruskal.test(dfsub$Distcen[dfsub$gender == "Weiblich" & dfsub$Intervention == "Recommendation"] ~ dfsub$Sourcetype[dfsub$gender == "Weiblich" & dfsub$Intervention == "Recommendation"])
```

*Based on the Kruskal Wallis test, average Distcens of women seeing a recommendation differ significantly for the different sourcetypes.*

```{r, echo = FALSE}
kruskal.test(dfsub$Distcen[dfsub$gender == "Weiblich" & dfsub$Intervention == "Default"] ~ dfsub$Sourcetype[dfsub$gender == "Weiblich" & dfsub$Intervention == "Default"])

kruskal.test(dfsub$Distcen[dfsub$gender == "Männlich" & dfsub$Intervention == "Recommendation"] ~ dfsub$Sourcetype[dfsub$gender == "Männlich" & dfsub$Intervention == "Recommendation"])

kruskal.test(dfsub$Distcen[dfsub$gender == "Männlich" & dfsub$Intervention == "Default"] ~ dfsub$Sourcetype[dfsub$gender == "Männlich" & dfsub$Intervention == "Default"])
```

*Neither for women seeing a default, nor for men seeing either intervention type, is there an effect of sourcetype on average Distcens.*

```{r, echo = FALSE}
kruskal.test(dfsub$Distcen[dfsub$gender == "Weiblich" & dfsub$Intervention == "Recommendation"] ~ dfsub$NonPolvsPolA[dfsub$gender == "Weiblich" & dfsub$Intervention == "Recommendation"]) 

lm <- (lm(Distcen ~ gender*RecvsDefD*NonPolvsPol, dfsub))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```

* *Based on the Kruskal Wallis test, there is a significant difference of Distcens between women seeing a recommendation by non-political/partisan vs. policial/partisan sources. This is a more aggregated effect than the effect before. Before, we saw that this effect is significant primarily because of the Distcens in the recommendation by partisan source groups (which has, by the way, 17 observations, i.e. there are 17 womeon that see a recommendation by a partisan source).*

## Gender and Reactance interaction per treatment
```{r, echo=FALSE}
ggplot(data = dfsub, aes(x = Reactance, y = Distcen, color = gender)) +
  facet_grid(~gender) +
  geom_jitter(shape=1) +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=2) +
  ylab("Average Distcen amount and mean Distcens") +
  xlab("Reactance score")
...
## Check average Distcens by treatment only for those that believed we cooperated with Julia Verlinden
```{r, echo=FALSE}
ggplot(dfbel, aes(treatment, Distcen)) +
  geom_boxplot() +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3)
kruskal.test(dfbel$Distcen ~ dfbel$treatment)
```

*The Kruskal Wallis test is not significant. This means that there are also no significant differences in Distcens for those that believed we cooperated with Julia Verlinden.*
...
```{r, echo=FALSE}
ggplot(data = df, aes(x = SourcetypeD, y = Distcen)) +
...
```{r, echo=FALSE}
wilcox.test(df$Distcen ~ df$warmGlowD)
...
lm <- (lm(Distcen ~ RecvsDefD * Sourcetype + warmGlowD, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
#lm <- (lm(Distcen ~ RecvsDefD * Sourcetype * warmGlowD, df))
#coeftest(lm, vcov = vcovHC(lm,type="HC1"))
...
* *Based on the Wilcoxon test, average Distcens differ between groups that feel or do not feel warm glow when contributing to climate change.*
...
* *Based on the robust OLS regression, warm Glow is highly positively significant as a predictor for average Distcens, when controlling for treatments and interactions, which themselves are all insignificant (Which they had been before anyway). When including warm Glow as a part of a tripple interaction term, every predictor becomes insignificant (not reported here).*
...
## Average Distcen efficiency-judgment differences (by treatment)
```{r, echo=FALSE}
ggplot(df, aes(retireEffD, Distcen)) +
  geom_boxplot() +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  xlab("Judgment of license retirement as efficient")
wilcox.test(df$Distcen ~ df$retireEffD) 
```

*Based on the Wilcoxon test we can confidently reject the null hypothesis of equal median Distcens (Assuming similar Distcenributions) of those that judge carbon license destruction as an efficient measure to fight climate change vs. does who do not.*

```{r, echo=FALSE}
ggplot(data = df, aes(retireEffD, Distcen, color = retireEffD)) +
  facet_wrap(~treatment) +
  geom_boxplot() +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  theme(axis.text.x = element_text(angle = 90, vjust=.8, size=10)) +
...
ggplot(data = df, aes(Sourcetype, Distcen)) +
  facet_grid(retireEffD~Intervention) +
  geom_boxplot() +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  theme(axis.text.x = element_text(angle = 90, vjust=.8, size=10))
lm <- (lm(Distcen ~ RecvsDef * Sourcetype * retireEffD, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```
...
## Average Distcen moral duty differences (by treatment)
```{r, echo=FALSE}
ggplot(df, aes(moralD, Distcen)) +
  geom_boxplot() +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  xlab("Judgment of license retirement as a moral duty")

wilcox.test(df$Distcen ~ df$moralD)
```
...
```{r, echo=FALSE}
lm <- (lm(Distcen ~ treatment + moralD, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
lm <- (lm(Distcen ~ RecvsDefD * Sourcetype * moralD, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```

* *When controlling for treatments, moral duty is a significant positive predictor for the Distcen amount.*
...
```{r, echo=FALSE}
ggplot(data = df, aes(moralD, Distcen, color = moralD)) +
  facet_wrap(~treatment) +
  geom_boxplot() +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
  theme(axis.text.x = element_text(angle = 90, vjust=.8, size=10)) +
...
*Reported income is relatively normally Distcenributed and does not differ with treatment.*

```{r, echo = FALSE}
ggplot(data = df, aes(x = income, y = Distcen)) +
  geom_jitter(shape=1)
lm <- (lm(Distcen ~ treatment+income, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
glm <- (glm(Donated ~ treatment+income, df, family = "binomial"))
coeftest(glm, vcov = vcovHC(glm,type="HC1"))
```

*Reported income is neither a significant predictor for the average Distcen amount, nor for the probability to donate anything.*
...
ggplot(df, aes(KnovsPol, Distcen)) +
  facet_grid(~RecvsDefD) +
  geom_boxplot()
```

*There is no interaction effect of Knowledge vs political source information and intervention type on the probability to donate anything. There is also no interaction on the average Distcen amount (regression not reported here).*

# Disaggregated reactance scores (4 categories)
```{r, echo=FALSE}
summary(lm(Distcen ~ treatment + as.factor(EmotionResp) + as.factor(ReactCompl) + as.factor(ResistInfl) + as.factor(ReactAdv), df)) 
#ggplot(data = df, aes(x = Sourcetype, y = Distcen, color = EmotionRespM)) +
#  facet_grid(EmotionRespM~RecvsDef) +
#  geom_boxplot() +
#  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=2) +
#  theme(axis.text.x = element_text(angle = 45, vjust=.5, size=10))
#lm <- (lm(Distcen ~ RecvsDef * Sourcetype * EmotionRespM, df))
#coeftest(lm, vcov = vcovHC(lm,type="HC1"))
#lm <- (lm(Distcen ~ RecvsDef * Sourcetype * ReactComplM, df))
#coeftest(lm, vcov = vcovHC(lm,type="HC1"))
#lm <- (lm(Distcen ~ RecvsDef * Sourcetype * ResistInflM, df))
#coeftest(lm, vcov = vcovHC(lm,type="HC1"))
#lm <- (lm(Distcen ~ RecvsDef * Sourcetype * ReactAdvM, df))
#coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```
...
kruskal.test(df$Distcen ~ df$demandEffect)
...
summary(lm(Distcen ~ treatment + demandEffectD, df))

describeBy(dfnoDE$Distcen, dfnoDE$treatment)
kruskal.test(dfnoDE$Distcen, dfnoDE$treatment)

summary(lm(Distcen ~ treatment, dfnoDE))
...
* *Kruskal Wallis Test suggests that experiencing a demand effect has an impact on Distcen amount.*
...
#summary(mhurdle(Distcen ~ moralD + retireEffD + NosvsSomeD + RecvsDefD | NosvsSomeD + RecvsDefD + belief + gender | 0, dfsub))
...
#summary(lm(Distcen ~ NosvsSomeD + RecvsDefD + belief + gender, dfsub))

#dfsub$beliefD <- ifelse(dfsub$belief > 0, 1, 0)
#summary(mhurdle(Distcen ~  moralD + retireEffD + NosvsSomeD * RecvsDefD * Reactance + gender + age + income | NosvsSomeD + RecvsDefD + belief + gender + age + income  | 0, dfsub, Distcen = "l")) # Distcen standard here is l, but changing it to n (normal) makes a huge difference. I do not really know what it means (however, l seems to be adequate (see below)) At the moment, this model does not account for correlation of the Distcenurbance of the good selection equation (hurdle 1) with that of the consumption equation (hurdle 2)
...
# a large part of the R² comes from beliefs (there is a huge problem of endogeneity with beliefs, also it is not clear whether Distcens explain beliefs or beliefs explain Distcens - it would possibly be better to keep them out)

#summary(mhurdle(Distcen ~  moralD + retireEffD + NosvsSomeD * RecvsDefD * Reactance + gender | belief + gender + age + income  | 0, dfsub, Distcen = "l"))
  ## /TRYOUT!

```

*Potentially interesting story. Is it possible to link regression to explain believes with regression to explain Distcens, of which belief is one independent variable? Probably overly complex.*
...
* the continuous variable of the Distcen conditional on having donated (Heckman?)
* the continuous variable of the Distcen conditional on having donated more than the median
...
#hurdle <- mhurdle(Distcen ~  moralD + retireEffD + NosvsSomeD * RecvsDefD * Reactance + gender + age + income | NosvsSomeD + RecvsDefD + belief + gender + age + income  | 0, dfsub, Distcen = "l", corr = NULL)
#hurdlecorr <- mhurdle(Distcen ~  moralD + retireEffD + NosvsSomeD * RecvsDefD * Reactance + gender + age + income | NosvsSomeD + RecvsDefD + belief + gender + age + income  | 0, dfsub, Distcen = "l", corr = "d")

#summary(glm(Donated ~  moralD + retireEffD + NosvsSomeD * RecvsDefD * Reactance + gender + age + income, dfsub, family = "binomial" ))

#lm <- (lm(Distcen ~ moralD + retireEffD + NosvsSomeD*RecvsDefD*Reactance + gender + age+ income, data = dfsub))

# how to get ROBUST SEs
#coeftest(glm, vcov = vcovHC(glm, "HC1")) # this is what stata would spit out

#lm <- (lm(Distcen ~ moralD + retireEffD + NosvsSomeD * RecvsDefD * Reactance + gender + age + income + belief, dfsub))
...
#stargazer(lm, lm,
      ...
      #    title="Regression Results", 
       #   align=TRUE, 
       #   dep.var.labels=c("Distcen amount"), 
       #   no.space=TRUE, 
...
summary(lm(Distcen ~ moralD + retireEffD + NosvsSomeD * RecvsDefD * Reactance + gender + age + income + belief, dfsub))
#plot(lm)
...
```
> Plot 1: Plots residuals vs. fitted values. If there is absolutely no heteroskedasticity, I should see a completely random, equal Distcenribution of points throughout the range of X axis and a flat red line. (In my example, heteroskedasticity seems to exist)
...
> In order to rectify Heteroskedasticity, one would need to rebuild the model with new predictors, or transform variables with, e.g., Box-Cox transformation. The latter procedure mathematically transforms a variable to make it approximate to a normal Distcenribution. This can be done for the dependent variable (can also be applied to IVs). Can only be applied to values bigger than 0 (if there are many zeros, we can add a constant to the variable)

> I should just use heteroskedasticity-robust standard errors! (MASS-library, Huber method: rlm() instead of lm)

```{r, echo=FALSE}
dfsub$Distcen1 <- dfsub$Distcen+6
hist(lm$resid) #looks reasonably normally Distcenirbuted
qqline(lm$resid)
skewness(lm$resid)

b <- boxcox(lm(Distcen1 ~ moralD + retireEffD + NosvsSomeD * RecvsDefD * Reactance + gender + age + income + belief, dfsub)) #because of the 0s, we have to do the transformation with Distcen+1
lamda <- b$x
lik <- b$y
bc <- cbind(lamda, lik)
bc[order(-lik),] # -0.06060606 maximizes the log-likelihood
lm2 <- lm((((Distcen1^(-06/99))-1)/(-06/99)) ~ moralD + retireEffD + NosvsSomeD * RecvsDefD * Reactance + gender + age + income + belief, dfsub) # not sure if this is really right
plot(lm2)
hist(lm2$resid) #looks reasonably normally Distcenirbuted
qqline(lm2$resid)
skewness(lm2$resid)
...
# Differently coded binary dependent variable (above or below median Distcen)
```{r, echo=FALSE}
...
ggplot(data = df, aes(x = DonatedMed, y = Distcen)) +
  facet_wrap(~treatment) +
  geom_jitter(shape=1) +
  xlab("Donated less or more than median") +
  ylab("Exact amount of Distcen") +
  theme(axis.text.x = element_text(angle = 45, vjust=.8, size=10))
...
# Change dependent variable Distcen to log(Distcen+1)
```{r, echo=FALSE}
# http://stats.stackexchange.com/questions/1444/how-should-i-transform-non-negative-data-including-zeros
df$logD <- log(df$Distcen+100)
log(2)
```

# Separate into two regressions (because bimodal): Logit then OLS (without zeros)
```{r, echo = FALSE}
#ggplot(data = df, aes(x = Distcenno0)) +
#  geom_histogram() +
#  stat_bin(binwidth = 1) +
#  xlab("Distcen amount for subjects that donated a positive amount")
```

*The Distcenribution of average Distcens excluding those that gave nothing is also not perfectly normally Distcenributed (because of the small peack at 1 Euro), but more so than the original variable. It is left skewed.*

```{r, echo = FALSE}
#ggplot(data = df, aes(x = Distcenno0)) +
#  geom_density()
## MIND HERE THAT THIS WOULD ALSO HAVE TO CHANGE WHEN I DECIDE TO LOOK AT THE DistcenANCE TO 5 INSTEAD OF THE Distcen AMOUNT
...
# Note that in a bimodal Distcenribution methods that look at the mean are not very good!!! https://communities.sas.com/t5/SAS-Statistical-Procedures/Mixed-model-bimodal-Distcenribution/td-p/56286
...
#lm <- lm(Distcenno0 ~ ..., dfsub)
```

# All tests and descriptive statistics with Distcenance to 5 instead of Distcen variable
...
## The Kruskal-Wallis test can be considered a special case of ordinal logistic regression, however. Moreover, OLR can handle a factorial structure, and does not require that your response data are normally Distcenributed, only that they are ordinal. This is likely to be your best option. On UCLA's excellent statistics help website, you can find guides to OLR in both R and Stata. 

## fit ordered logit model and store results 'm'
summary(polr(as.factor(as.character(Distcen)) ~ moralD + retireEffD + NosvsSomeD * RecvsDefD * Reactance + gender + belief, data = dfsub, Hess=TRUE))
