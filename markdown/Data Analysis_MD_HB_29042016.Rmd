---
title: "Data Analysis_HB_`r, format(Sys.time(), '%d%m%Y'))`"
...
ggplot(data = dfsub, aes(gender, Donation, color = gender )) +
  facet_wrap(~treatment) +
  geom_boxplot() +
  geom_text(data=df.nu, aes(x=1, y = 6, label=x), parse=TRUE) +
  geom_text(data=df.nu, aes(x=2, y = 6, label=x), parse=TRUE) +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3)
```
