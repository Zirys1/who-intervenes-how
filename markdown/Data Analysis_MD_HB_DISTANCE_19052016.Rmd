---
title: "Data Analysis_HB_20052016 - DIST"
...
```{r, echo=FALSE}
ggplot(data=df, aes(x = trustPolM, y = Dist, color = RecPolvsDefPol)) +
  facet_wrap(~RecPolvsDefPol) +
  geom_boxplot() + 
  geom_jitter(shape=1) +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
...
lm <- (lm(Dist ~ RecPolvsDefPol * trustPolM, df))
coeftest(lm, vcov = vcovHC(lm,type="HC1"))
```
...
wilcox.test(df$Dist[df$treatment == "RecPol" | df$treatment] ~ df$trustPolM[df$treatment == "RecPol"], exact = FALSE, alternative = "two.sided")
wilcox.test(df$Dist[df$treatment == "DefPol"] ~ df$trustPolM[df$treatment == "DefPol"], exact = FALSE, alternative = "two.sided")
...
ggplot(data=df, aes(x = conformityD, y = Dist, color = RecPolvsDefPol)) +
  facet_wrap(~RecPolvsDefPol) +
  geom_boxplot() +
  geom_jitter(shape=1) +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
...
ggplot(data=df, aes(x = conformityM, y = Dist, color = RecPolvsDefPol)) +
  facet_wrap(~RecPolvsDefPol) +
  geom_boxplot() + 
  geom_jitter(shape=1) +
  stat_summary(fun.y = mean, colour="darkred", geom="point", shape=18, size=3) +
...
```{r, echo = FALSE}
ggplot(data=df, aes(x = green, y = Dist, color = RecParvsDefPar)) +
