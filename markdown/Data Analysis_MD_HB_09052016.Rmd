---
title: "Data Analysis_HB_11052016"
...
load("Data_09052016.RData") # without those who already knew Julia Verlinden
```
...
*30.79% of the participants donate nothing. 15.25 donate 1â‚¬, 23.73% donate 2â‚¬,*
...
```
*The decision of whether or not to donate depends on Reactance (based on the ChiÂ² Test. However the test is only slightly significant (p < .1). The logistic regression model is not significant*
...
```
*The ChiÂ²-test statistic (simulated p-value) is significant, indicating that the decision whether or not to donate anything does depend on the EAI.*
...
ggplot(data = dfsub, aes(x = Donation)) + # selection model
  geom_histogram()+
  facet_grid(~Donated)
```
...
> I should just use heteroskedasticity-robust standard errors!
